...
def getvalues(): # Function that read user inputs and return them to use them in initializing our model 
    ...
    epochsValue = int(epochsEntry.get())
    biasCheck = int(biasVar.get())
    msg = messagebox.showinfo("Done", "Done...Now learning!")
    return featureBoxvalue1, featureBoxvalue2, classComboValue, lrateValue, epochsValue, biasCheck 
...
lrateLabel.place(x=80, y=173)

epochsVar = StringVar()
epochsLabel = Label(frame, textvariable=epochsVar)
epochsVar.set("Enter number of epochs:")
epochsLabel.place(x=50, y=205)
...
lrateEntry.place(x=200, y=173)

epochsEntry = Entry(frame)
epochsEntry.place(x=200, y=205)
...
def StartModel(): # function for initializing model by calling getvalues function and check what classes selected by user and also convert bais Availability to true and false
    ...
    F1, F2, ClassLabel, lrateValue, epochsValue, biasCheck = getvalues()
    # visualG(F1,F2)
    # If you want to show selected Features applied on the 3 classes 
    # comment code starting from line 130 >>> line 152
    bs = False
    C1 = C2 = None
    if biasCheck == 1:
        bs = True
    if ClassLabel == 'C1 & C2':
        ...
        C1 = "c2"
        C2 = "c3"
    TestModel = SLPModel(lrateValue, epochsValue, bs, F1, F2, C1, C2)
    TestModel.Epochs()
    TestModel.TModel()

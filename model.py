...
class SLPModel:
    ...
    eta = TruePositive = TrueNegative = AllPositive = Min = Max = 0
    NumberOfEpochs = None
    ...
    def __init__(self, LearningRate, Epochs, baisAva , Feature1Name, Feature2Name, class1, class2):
        ...
        self.cl1 = class1
        self.cl2 = class2
        self.bais = round(random.uniform(-1, 1), 2) # 0.21
    ...
    def Epochs(self): # Function for Epochs get every row and its target and call Model Function after looping over whole train data check if WeightsChange == false then no Weights changed
        for e in range(self.NumberOfEpochs):
            self.WeightsChange = False
            for i in range(self.TL.count()): 
                row = self.TD.iloc[i] 
                target = self.TL[i]
                self.Model(row, target)
            if self.WeightsChange == False:
                break
    ...
    def Model(self, X_i, T_i): # Function for our SLP first calculate output then apply signum function to get if it is -1 or 1 
        ...
        output = self.CalculateOutput(X_i[0],X_i[1])
    ...
    def CalculateOutput(self, feature1Value, feature2Value): # Function to Calculate Output by using feature1 and feature2 values
        ...
        if self.BaisControl == True:
           return  int((self.weight1 * feature1Value) + (self.weight2 * feature2Value) + self.bais)
        else:
           return  int((self.weight1 * feature1Value) + (self.weight2 * feature2Value))
